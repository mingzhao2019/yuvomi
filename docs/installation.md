...
A calendar sync does not run during OAuth: connecting only discovers account metadata. It starts
after an admin saves the selected resources and triggers **Full sync**, on the shared sync run
(`SYNC_INTERVAL_MINUTES`, 15 minutes by default), and whenever an admin triggers it manually. Microsoft
