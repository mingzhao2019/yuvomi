...
| done_by_user_id | INTEGER | FK → Users (SET NULL), nullable — **who did it**, when somebody was named (migration v224, #1205) |
| completed_at | TEXT | ISO 8601 UTC, default now |

**Two people, because there are two questions (migration v224, #1205).** `user_id` keeps its meaning
...
### Shopping List Changes (migration v206)
...
| recurrence_parent_id | INTEGER | Self foreign key to the recurring master with ON DELETE CASCADE, nullable (migration v204) |
| recurrence_id | TEXT | Original recurrence slot of a linked replacement, not its possibly moved displayed start, nullable (migration v204) |
| overridden_fields | TEXT | Canonical JSON array naming the fields intentionally owned by a linked replacement, nullable (migration v204) |
...
| user_modified | INTEGER | 0/1 — marks a mirrored event as locally edited for the general event fields |
| color_modified | INTEGER | 0/1 — separately marks an explicit local colour choice, including deliberately clearing the colour (migration v171) |
...
| target_outlook_account_id | INTEGER | Outlook account ID for the bidirectional Outlook target (migration v154), nullable. Mutually exclusive with the other target columns |
| target_outlook_calendar_id | TEXT | Microsoft Graph calendar ID for the bidirectional Outlook target (migration v134), nullable |
...
**Completed calendar events remain visible as acknowledgements:** calendar completion is personal and stored per event occurrence in `calendar_event_completions`, so a completed event still appears in Key dates while its date remains eligible under the same overdue-grace rules. The row carries the completed/archive-restore marker and struck title, while the original date and countdown remain visible. For recurring events the marker follows the next occurrence key; a completed task, or an archived task, remains excluded from Key dates.
...
**Linked local occurrence overrides (migration v204, #975).** The current only-this edit path stores a concrete replacement row with `recurrence_parent_id`, `recurrence_id`, and `overridden_fields`. The parent plus original slot is the durable identity, so moving the displayed start does not change which occurrence is being edited. This model also subsumes the existing imported `${uid}::${recurrenceId}` identity: an imported override is already a concrete rule-less event row, and a future migration can locate its master and populate the same parent and original-slot columns. It does not need a second override table or a second read model. The current day-based key and unique `(recurrence_parent_id, recurrence_id)` index allow exactly one overridden original slot per series per day; importing multiple occurrences on the same day would require widening the key, index, and date-key validation first. Migration v204 deliberately performs no such import migration and does not heuristically relink historic detached local edits, because their original slot cannot be recovered unambiguously.

Eligibility for linked overrides is server-owned and local-only. A series must have a recurrence rule, have `external_source = 'local'`, carry no external UID, object URL, calendar reference, subscription, provider link, or Google, CalDAV, or Outlook outbound target, and have no birthday, name-day, housekeeping, or other generated owner. `is_local_recurring_series` describes local origin, independently of `can_override_occurrence`; an explicit outbound target is nevertheless an external whole-series boundary for occurrence scopes, because EXDATE cannot be propagated to the provider. A locally owned series with an Outlook auto-sync exposure or a birthday, name-day, housekeeping, or other generated owner may expose `can_detach_occurrence` to visible actors instead: all three scopes retain the legacy standalone-event plus EXDATE / truncate-plus-successor workflow. That fallback intentionally retains its existing multi-request partial-failure window, while linked overrides are atomic. Imported series keep whole-series behavior and never use that fallback. Occurrence mutations use the same row-visibility authorization as the established whole-series PUT/DELETE path, except that the legacy EXDATE route remains owner/admin gated for compatibility. Apple auto-sync excludes linked children and masters with linked children, but a deletion-only EXDATE does not remove a series from its existing outbound behavior. API capability fields are presentation data; the server repeats authoritative visibility and eligibility checks before writing.
...
The migration v85 behavior recorded below remains historic context. Where it describes detached only-this edits or separate child and EXDATE requests, migration v204 supersedes it for eligible local series.
...
### Outlook Accounts
Connected personal Microsoft accounts (outlook.com / M365 Family) for the bidirectional Outlook
Calendar sync (initial account schema migration v154, sync state migration v168). Multi-account like
CalDAV Accounts; OAuth tokens live per account row, **not** in
...
Events imported from Outlook remain linked to their Graph object; editing such an event locally marks
the link dirty and sends the edit back to Outlook.

### Outlook Calendar Selection
Calendars of a connected Outlook account, loaded from Graph on connect (refreshable); `enabled`
calendars are imported and appear as push targets. Newly discovered calendars start **disabled**: the connect flow
...
| enabled | INTEGER | 0/1, imports the calendar and offers it as a push target |
| sync_start_date | TEXT | nullable absolute `YYYY-MM-DD`; null means today minus six months |
| sync_range_start / sync_range_end | TEXT | effective Graph delta-query window cached for the cursor |
| sync_cursor | TEXT | opaque Graph `calendarView/delta` cursor, nullable |
| last_inbound_sync / sync_error | TEXT | inbound bookkeeping, nullable |
| UNIQUE | | (account_id, calendar_id) |

### Outlook Event Links
Bidirectional link state per (event, account) — one event can sync into several accounts' calendars.
**Deliberately no FK on `calendar_events`**: the row must survive deletion of the local event so the
next sync can delete the remote event in Outlook (tombstone). Imported events use
`external_source='outlook'`; locally pushed events remain `local`, and both kinds use the same link
state for change detection and outbound work.
...
| content_hash | TEXT | SHA-256 of the last local/common payload — unchanged events are no-ops |
| outlook_change_key | TEXT | Graph change key after the last known remote state/write, nullable |
| link_type | TEXT | `push` or `inbound` |
| remote_content_hash | TEXT | SHA-256 of the last known Outlook payload |
| outbound_dirty / outbound_attempts | INTEGER | local edits waiting to be sent and retry count |
| pending_delete / remote_missing | INTEGER | local tombstone and remote-delete recovery state |
| local_snapshot / series_master_id / last_inbound_at | TEXT | conflict/recurrent-series bookkeeping |
| last_pushed_at / last_error | TEXT | Bookkeeping, nullable |

**Bidirectional reconciliation:** every enabled calendar uses Graph `calendarView/delta` with an
opaque cursor and a fixed date window. New remote events are imported as `external_source='outlook'`;
remote updates flow into an unchanged local mirror, and local edits flow back with `If-Match` against
the last known `changeKey`. If both local and remote payloads changed since the common state, the
event stays unchanged and a pending row in `outlook_calendar_conflicts` exposes both snapshots for
an explicit `local` or `remote` choice. Graph `@removed` entries remove unchanged local mirrors;
local deletes use the link tombstone to delete Outlook. Events without a link row are not imported
unless their calendar is enabled, and changing the import start date clears only that calendar's
cursor — existing local events are retained.
...
counts failed pushes under the same five-attempt limit. Deliberately **not** `user_modified` or
`color_modified` — those flags describe local ownership and colour protection, while
`outbound_dirty` is a queue, set on edit and cleared after the push. Only changes to
...
### Display Accounts (migration v225, v226, #1208, decided in #913)
...
| display_devices | cookie_refreshed_at | TEXT | ISO 8601, nullable - separate from `last_seen_at`; tracks when the persistent device cookie was last re-dated (migration v226) |
...
| created_by | INTEGER | FK → Users (CASCADE delete), NOT NULL |
| expires_at | TEXT | nullable, `YYYY-MM-DD` (migration v228) |
| expiry_reminder_days | INTEGER | nullable = no reminder; 0–365 when set, range-validated in the route (migration v228) |
...
**Expiry date and reminder (v228):** a document may carry an optional
...
### Waste Types (migration v207, #1063)
...
### Waste Schedules (migrations v207, v214, #1063)
A weekly, fixed-day-of-month, or ordinal-weekday-of-month (migration 214, #1063 Phase 9, e.g. "2nd
...
### Waste Schedule Overrides (migration v207, #1063)
...
### Waste One-Off Pickups (migration v207, #1063)
...
### Waste Sources (migrations v209, v212, #1063)
...
date, including labels the household chose to ignore). Migration v212 (Phase 7) widened `kind` to add
...
### Waste Source Mappings (migration v210, #1063)
...
### Waste Imported Pickups (migration v211, #1063)
...
### Waste Reminder Settings (migration v213, #1063 Phase 8)
...
### Waste Reminder Entries (migration v213, #1063 Phase 8)
...
### Waste ICS feed and per-type feed selection (migration v215, #1063 Phase 10)
...
### Waste in global search (migration v216, #1063 Phase 10)
...
deletion. Migration 230 adds `remind_goal` and `remind_next_start`, both off by
...
| mood | TEXT | **Legacy, frozen as of migration 221.** Single feeling key; no longer written - see `cycle_day_log_feelings` below. Still returned read-only and accepted as input (treated as `feelings: [mood]`) for older clients. |
| cervix_mucus | TEXT | nullable (migration 220) - `dry` \| `sticky` \| `creamy` \| `watery` \| `eggwhite`, route-enforced like `basal_temp_unit`. Owner-only, see below. |
| lh_test | TEXT | nullable (migration 220) - `negative` \| `positive`. Owner-only, see below. |
| pregnancy_test | TEXT | nullable (migration 220) - `negative` \| `positive`. Owner-only, see below. |
| intimacy | TEXT | nullable (migration 220) - `protected` \| `unprotected` \| `solo`. Owner-only, see below. |
| note | TEXT | |
| visibility | TEXT | `private` \| `family`, default `private` |
...
| contraception | TEXT | nullable (migration 222) - `none` \| `pill` \| `hormonal_iud` \| `copper_iud` \| `implant` \| `injection` \| `patch` \| `ring` \| `condom` \| `other`. The hormonal subset (`pill`, `hormonal_iud`, `implant`, `injection`, `patch`, `ring`) suppresses fertile-window/ovulation prediction (`fertilitySuppressed: 'contraception'`), with an explanatory note where the fertile-window tile would be - hidden-but-explained, never silently broken. |
| perimenopause_mode | INTEGER | 0/1, default 0 (migration 222) - next period becomes a min-max date range from the recent plausible gaps (`nextStartRange`), the Regular/Irregular judgement is suppressed (irregularity is expected, not an alarm) |
| show_pms | INTEGER | 0/1, default 1 (migration 222) - toggles the derived PMS-window shading on the calendar; the window itself is computed, never stored (`pmsWindow()`) |
| notify_partner_user_id | INTEGER | nullable (migration 222), FK → Users (SET NULL) - owner-opt-in partner reminder; must be another household member |
| notify_partner_days_before | INTEGER | nullable, 0-14 (migration 222) - lead time for the partner's reminder |
| created_at / updated_at | TEXT | ISO 8601, default now |

**`cycle_day_log_feelings`** (migration 221) - multi-select feelings for a day log, normalized out
of the legacy scalar `mood` column the same way migration 182 normalized symptoms, but the backfill
...
**Partner period reminder** (migration 223) widens `cycle_reminder_anchors.kind` with
...
**Preventive care & vaccinations (migration 229):** one model for both a
...
- **Live updates while the list is open (migration v206):** an open list hears within about
...
- **Outlook bidirectional sync (Microsoft Graph, migrations v154/v168):** Sync personal Microsoft accounts (outlook.com / M365 Family) with Yuvomi through the `/consumers` OAuth endpoint and the Graph API. Enabled Outlook calendars import events from six months before today by default; each calendar stores an optional absolute `sync_start_date`, its fixed delta-query range, and an opaque `calendarView/delta` cursor. Changing the start date resets only that cursor and retains existing local events. Local events with an Outlook target are pushed, while Outlook-only events in enabled calendars become `external_source='outlook'` mirrors. Both directions use link-level hashes and Graph `changeKey`/`If-Match`; simultaneous edits are held in `outlook_calendar_conflicts` with local and remote snapshots until an admin chooses the local or remote version. Remote deletions remove unchanged mirrors, and local deletions use tombstones to delete Outlook. Auto-sync remains visibility-aware and excludes other provider mirrors to avoid duplicates. Recurrence maps the supported Yuvomi RRULE subset onto Graph's `pattern`/`range`; unsupported remote patterns become one-off events. Outlook Calendar attendees, calendar reminders, attachments and colors are not synchronized. OAuth refresh-token expiry still marks `needs_reauth`. **Microsoft To Do (migration v164):** each enabled list is represented in the Tasks sidebar and uses its own Delta cursor for normal polling. The manual sync always starts a fresh full list round, and scheduled polling starts one at least every six hours per list; a full round reconciles the current remote task set and removes only clean local mirrors that are absent remotely. The last full-round timestamp is persisted with the list (migration v165). To Do `importance` is intentionally reduced to two states: Graph `high` (the star/Important state) maps to Yuvomi `high`, and every other provider value maps to Yuvomi `none`; Yuvomi `high`/`urgent` maps back to Graph `high`, while all other priorities map to Graph `normal`. `isReminderOn`/`reminderDateTime` synchronize the task creator's personal reminder in both directions. Supported To Do `recurrence` patterns (daily, weekly, absolute monthly and absolute yearly, including end date/count) map to Yuvomi's RRULE subset and are sent back to Graph; unsupported relative patterns remain one-off mirrors. Microsoft owns the next occurrence after a recurring item is completed, so Yuvomi does not create a duplicate local follow-up. Graph `dateTimeTimeZone` values are interpreted in the provider-declared zone and carried over as an instant using the household zone; the explicit UTC value used on the Graph wire does not alter the household display. Microsoft To Do `steps`/`checklistItems` are deliberately excluded: they are nested checklist entries without Yuvomi task identity, visibility, permissions or parent-task relationships, whereas a Yuvomi subtask is a real task.
...
- **Event color sync (Discussion #427, #891, #899):** Each provider preserves per-event colors, not just the calendar color. Inbound, Google's `colorId` is resolved to a hex value via the event color palette (`colors.get`, cached 24 h), and the iCalendar `COLOR` property (RFC 7986 — CSS3 name or hex) is read for CalDAV, Apple, and ICS subscriptions. Only an explicit provider event colour is stored in `calendar_events.color`; the provider calendar/subscription colour is exposed separately as read-only `cal_color`. A `NULL` event colour therefore means “no own colour”: display resolution uses the primary assignee's colour, then `cal_color`, then neutral grey. Outbound to Google, a local event's hex color is mapped to the nearest of Google's 11 event `colorId`s; CalDAV/Apple use the nearest CSS Color Level 3 name because RFC 7986 does not accept arbitrary hex values. Locally recolored events use the separate `color_modified` flag: a resync overwrites an event's colour only while `color_modified = 0`, while `user_modified` continues to describe other local event edits. Omitting `color` in a PUT leaves it untouched; sending `color: null` deliberately clears it and, when mirrored, removes the provider colour. The `COLOR`↔hex mapping lives in `server/utils/ical-color.js`.

  The colour picker starts with an explicit “inherit” swatch. Choosing it writes `color: null`; choosing a palette colour writes an own colour, and an untouched colour is not sent as a destructive fallback. The picker also keeps an out-of-palette colour as an extra swatch and compares hex values case-insensitively. The shared `resolveEventColor()` rule is used by the calendar, dashboard, and countdown data so the same event does not receive different colours on different surfaces.
