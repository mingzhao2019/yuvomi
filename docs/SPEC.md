...
**Applying a default assignee to events already imported (#1154):** the sync stays new-only, but an admin can run a one-off backfill from Settings → Sync. `GET /api/v1/calendar/external-calendars/default-assignee-backfill` returns `{ count }` for the confirmation, and `POST` on the same path applies it and returns `{ assigned }`. It covers every `external_calendars` row of every account that carries a default assignee and fills only events still mirrored from that calendar, that arrived inbound rather than being created locally and pushed out, and with **no assignment at all** (`assigned_to IS NULL` and no `event_assignments` row), so a manual assignment always wins. It writes through `setEventAssignments()`, so the author's future reminders fan out to the new assignee and restricted attachments open to them; inherited reminders whose time has passed are dismissed. The POST requires `expected_count`, answers 409 with the current count when the candidate set changed, processes only the confirmed snapshot, re-checks each event while writing, and works in batches of 50 with a yield between them. ICS subscriptions are not included, and an orphaned default assignee is skipped.
