...
**Archived tasks remain visible while their countdown is eligible:** archiving is separate from completion, so an open task with `countdown = 1` and a due date continues to appear through the same date and overdue-grace rules. The row carries an explicit archived marker and still opens the task detail, where it can be reviewed or restored. Completed tasks remain excluded. Calendar events have no parallel archive axis.
