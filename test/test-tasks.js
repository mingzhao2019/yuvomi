...
test('Task Lists sind eine dauerhafte Navigation statt einer Filter- oder Gruppierungsachse', () => {
  ...
  assert(source.includes('function activateTaskListSource('), 'Quellen müssen als eigener Navigationsbereich wählbar sein');
  assert(source.includes('focusedSource === group.key'), 'Desktop zeigt nach der Quellenwahl nur deren konkrete Listen');
  assert(source.includes("key: 'all', provider: 'all'"), 'es gibt genau einen globalen Alle-Bereich');
  assert(!source.includes('COLLAPSED_TASK_SOURCES_KEY'), 'pro Quelle darf kein separater Klappzustand zurückbleiben');
  assert(!source.includes('taskSourceToggle'), 'Quellen brauchen keinen zusätzlichen Klappknopf');
  assert(!source.includes('TASK_LIST_ALL_OPTION'), 'provider-spezifische Alle-Optionen dürfen nicht zurückkehren');
