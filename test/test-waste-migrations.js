...
// -------------------------------------------------------------------------
// Upgrade path: migrations < 207 leave no Waste tables; 207 alone adds them
// without touching anything else.
// -------------------------------------------------------------------------

test('upgrade: a database migrated only through 206 has no Waste tables, and pre-existing tables survive 207', () => {
  const preWaste = buildMigratedDatabase(MIGRATIONS.filter((m) => m.version < 207));
  ...
  applyMigration(preWaste, MIGRATIONS.find((m) => m.version === 207));
