 ...
 * Kalender heißt: kein Kalender liefert etwas - so läuft der Outbound-Push
...
describe('#1270 - der Umzug nimmt die unangetastete Standard-Zuweisung mit', () => {
  ...
  it('die Erinnerung folgt der Zuweisung, eine vergangene aber nicht als Meldung', async () => {
    const refA = externalCalendar('caldav', CAL_A, 'Kalender A', ANNA);
    const refB = externalCalendar('caldav', CAL_B, 'Kalender B', BEN);
    seedCaldavAccount();
    ...
    // Custom's provider policy deliberately reconciles a no-VALARM event with
    // the owner's personal defaults during inbound sync. Test the assignment
    // fan-out directly here so this assertion does not replace that policy
    // with upstream's no-op behavior.
    assert.equal(reassignDefaultOnCalendarMove(db, imported.id, {
      fromCalRefId: refA,
      toCalRefId: refB,
      toDefaultUserId: BEN,
    }), true);
...
// hinausgepushten Termin ausdrücklich aus; der Umzug tat es nicht und stellte
// die Handarbeit auf die Person von B um.
//
// Gefahren wird der ECHTE Weg: der Outbound-Push übergibt die Zeile an den
// Sync (oikos-UID, `calendar_ref_id`), der Inbound hängt sie danach um.
// --------------------------------------------------------

describe('#1270 - ein hinausgepushter Termin behält seine Zuweisung', () => {
  beforeEach(resetTables);
  ...
  it('CalDAV: der eigene Termin, von Hand zugewiesen und hinausgepusht, behält sie', async () => {
    externalCalendar('caldav', CAL_A, 'Kalender A', ANNA);
    externalCalendar('caldav', CAL_B, 'Kalender B', BEN);
    seedCaldavAccount();
    const accountId = Number(db.prepare('SELECT id FROM caldav_accounts').get().id);
    ...
    // ECHTER Outbound: kein Kalender liefert etwas, der Push läuft.
    await caldavSync({ createClient: caldavClient(null) });
    const uid = `oikos-${id}@oikos.local`;
    const pushed = rowOf(id);
    assert.equal(pushed.external_source, 'caldav', 'der Push hat die Zeile übergeben');
  ...
  it('Apple: die oikos-UID allein hält die Zuweisung, ohne jedes Ziel', async () => {
    externalCalendar('apple', CAL_A, 'Kalender A', ANNA);
    externalCalendar('apple', CAL_B, 'Kalender B', BEN);

    // Der Apple-Legacy-Outbound lädt JEDEN lokalen Termin in den ersten
    ...
    const uid = `oikos-${id}@oikos.local`;
    const pushed = rowOf(id);
    assert.equal(pushed.external_source, 'apple', 'der Push hat die Zeile übergeben');
    assert.equal(pushed.external_calendar_id, uid, 'und ihr die oikos-UID gegeben');
    assert.equal(Number(pushed.calendar_ref_id), refOf(CAL_A), 'sie liegt in Kalender A');
  ...
  it('Google: der gewählte Zielkalender hält die Zuweisung', () => {
    const refA = externalCalendar('google', GCAL_A, 'Kalender A', ANNA);
    const refB = externalCalendar('google', GCAL_B, 'Kalender B', BEN);

    // Der Stand, den der Google-Outbound nach dem Push schreibt
    // (google-calendar.js: external_source='google' + calendar_ref_id, das
    // gewählte Ziel bleibt stehen). Von Hand an Anna, die auch A vergibt.
    const id = Number(db.prepare(`
      INSERT INTO calendar_events
        (title, start_datetime, end_datetime, created_by, assigned_to,
  ...
  it('Google: ein Termin von vor Migration 47 bleibt außen vor', () => {
    const refA = externalCalendar('google', GCAL_A, 'Kalender A', ANNA);
    const refB = externalCalendar('google', GCAL_B, 'Kalender B', BEN);
