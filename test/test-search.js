...
db.exec(MIGRATIONS_SQL[204]); // linked occurrence overrides
...
// Migration 207: waste_types (only the table search reads from).
db.exec(MIGRATIONS_SQL[207]);
// Migration 216: FTS triggers + backfill for waste_types.
db.exec(MIGRATIONS_SQL[216]);
...
test('Migration 44 legt FTS5-Tabelle und Trigger an, Backfill leer (Seed danach)', () => {
  ...
  assert(triggers.n === 24, `Erwartet 24 Trigger (15 aus Mig. 44 + 6 aus Mig. 66 + 3 aus Mig. 209), erhalten ${triggers.n}`);
});
