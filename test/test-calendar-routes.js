...
test('local outbound targets stay external for occurrence scopes', async () => {
  for (const target of [
    ...
    assert.equal(event.can_detach_occurrence, false);
    assert.equal((await call('POST', `/${id}/exceptions`, { body: { date: '2042-02-03' } })).status, 400);
...
test('POST /:id/exceptions — nur Eigentümer oder Admin dürfen EXDATE setzen', async () => {
  const serie = insertEvent({ title: 'EXC-OK', start_datetime: '2042-03-01T09:00', recurrence_rule: 'FREQ=DAILY', created_by: MARIA.id });
  const denied = await call('POST', `/${serie}/exceptions`, { actor: TOM, body: { date: '2042-03-05' } });
