...
// --------------------------------------------------------------------------
// Migration v221: cycle_day_log_feelings-Backfill aus der alten mood-Spalte
// --------------------------------------------------------------------------

const V221 = MIGRATIONS.find((m) => m.version === 221);

function seedPreV221() {
...
function appliedV221() {
  const db = seedPreV221();
  // Anders als V182 ist v221s `up` reines SQL (kein Funktions-up) - die
  // Zerlegung passiert hier nicht zeilenweise in JS, sondern per SQL-INSERT
  // ... SELECT, siehe Migration 221 in server/db.js.
  db.exec(V221.up);
  return db;
}

test('v221 legt cycle_day_log_feelings mit den erwarteten Spalten an', () => {
  const db = appliedV221();
...
test('v221 befuellt genau eine Zeile je Tages-Log mit gesetztem, gueltigem mood-Wert', () => {
  const db = appliedV221();
  const rows = db.prepare(
...
test('v221 erzeugt keine Zeile fuer NULL oder leere mood-Spalten', () => {
  const db = appliedV221();
...
test('v221 normalisiert Gross-/Kleinschreibung beim Backfill ("Good" -> "good")', () => {
  const db = appliedV221();
...
test('v221 uebernimmt keinen Wert ausserhalb der sieben Feelings-Schluessel ("tired" bleibt nur in mood)', () => {
  const db = appliedV221();
...
test('v221 laesst die alte mood-Spalte unveraendert stehen (kein Rebuild, keine Loeschung)', () => {
  const db = appliedV221();
...
test('v221 legt einen Index auf day_log_id an', () => {
  const db = appliedV221();
