...
// --------------------------------------------------------
// 1b) Heilung der Migration v205 (#1140) - bewusst gegen die ECHTEN
...
//     schrieben bis v205 das rohe 'Sonstiges' statt des Keys 'misc', und ohne
//     diese Probe haelt kein Test das UPDATE der Migration selbst fest.
// --------------------------------------------------------
{
  const V205 = db.MIGRATIONS.find((m) => m.version === 205);
  test('Migration v205 existiert und fasst die Sonstiges-Kategorie an', () => {
    assert(V205, 'Migration 205 fehlt in db.js MIGRATIONS');
    assert(/Sonstiges/.test(V205.up), 'v205 muss die Kategorie Sonstiges anfassen');
  });

  // Vor-v205-Stand: Schema + Kategorien-Seed (v84), dann Zeilen, wie die
  ...
  test('v205 heilt Sonstiges-Kontakte auf misc, andere Kategorien bleiben', () => {
    mig.exec(V205.up);
