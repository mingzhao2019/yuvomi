/**
 * Test: Verlorene Ledger-Zeilen aktiver Ausgaben werden neu aufgebaut (Migration v235, #1382)
 * Zweck: `expense_ledger_entries.created_by` ist ON DELETE CASCADE. Ein PUT
 *        /expenses/:id stempelte die Zeilen mit der BEARBEITENDEN Person; wurde
 *        deren Konto geloescht, nahm die Kaskade die Zeilen mit, und die weiter
 *        aktive Ausgabe fiel still aus allen Salden. v234 hat den Autor der
 *        noch vorhandenen Zeilen korrigiert, die schon verlorenen kann ein
 *        UPDATE nicht zurueckholen. v235 baut sie aus
 ...
 *           sich geaendert. Dann v235 NICHT anfassen - sie laeuft auf
 ...
 *           neue Regel anpassen (bzw. auf den Stand von v235 festnageln).
...
const db = dbmod.get();
const migration235 = dbmod.MIGRATIONS.find((m) => m.version === 235);
...
test('Migration v235 existiert und laeuft als Funktion', () => {
  assert.ok(migration235, 'MIGRATIONS enthaelt v235');
  assert.equal(typeof migration235.up, 'function');
});
...
test('Upgrade ab v225 durch den echten Runner baut die Zeilen auf und verbucht v235', () => {
  ...
  try {
    ...
    assert.ok(fresh.prepare('SELECT 1 FROM schema_migrations WHERE version = 235').get(), 'v235 verbucht');
    ...
    assert.equal(fresh.prepare("SELECT COUNT(*) AS n FROM sqlite_temp_master WHERE name = '_v235_missing'").get().n, 0, 'Hilfstabelle geraeumt');
  } finally {
