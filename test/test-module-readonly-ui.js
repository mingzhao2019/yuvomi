...
test('Kalender-Detailansicht: Löschen, Zurücksetzen und Bearbeiten fallen weg, die Karte bleibt', () => {
  ...
  assert.match(
    CAL_CODE,
    /ev\.external_source === 'ics'\s*&&\s*\(ev\.user_modified === 1 \|\| ev\.reminder_suppressed === 1\)\s*&&\s*!readOnly\(\)/,
...
function reminderSection(html) {
  ...
  if (start < 0) return '';
  // custom adds the task document-attachment field after reminders. It is a
  // task permission concern, not a calendar permission concern, so do not
  // include its file input in the reminder controls below.
  const boundaries = [
    html.indexOf('<div class="form-group doc-attach"', start),
    html.indexOf('<div id="task-form-error"', start),
