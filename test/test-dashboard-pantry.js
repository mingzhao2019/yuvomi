...
test('Registrierung: ein Bestandslayout ohne pantry bekommt die Kachel ausgeblendet dazu', () => {
  ...
  // bleibt, ist die Default-Position des Neuzugangs: hinter dem custom-Asset-
  // Widget, das in WIDGET_IDS zwischen Einkauf und Vorrat steht.
  const ids = merged.map((w) => w.id);
  assert.equal(ids.indexOf('pantry'), ids.indexOf('assets') + 1, 'die Ergänzung steht an ihrer Default-Position');
});
