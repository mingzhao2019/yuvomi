...
import { sync, getCountryCatalog, getForRange, getCountries, getSubdivisions, getGroups, __setFetchImpl, __test } from '../server/services/holidays.js';
...
test('getCountries: keeps the built-in China option when OpenHolidays is unavailable', async () => {
  __setFetchImpl(async () => { throw new Error('network down'); });
  assert.deepEqual(await getCountries(), [
    { isoCode: 'AU', name: 'Australia', schoolHolidays: false },
    { isoCode: 'BR', name: 'Brazil', schoolHolidays: false },
    { isoCode: 'CA', name: 'Canada', schoolHolidays: false },
    { isoCode: 'CN', name: 'China' },
