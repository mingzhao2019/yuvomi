...
import { resolveReminderPreset, remindAtFromPreset, parseRemindAtAsUtc, wallTimeToInstant, wallTimeToStoredUtc } from '/utils/reminder-offset.js';
...
function saveRecentFilter(filters) {
  ...
  if (!set.status.length && !set.priority.length && !set.assigned_to.length
      && !set.category.length && !set.tags.length) return;
...
export async function render(container, { user }) {
  ...
  try {
    const [tasksData, metaData, preferencesData, taskListsData, displayPeople] = await Promise.all([
