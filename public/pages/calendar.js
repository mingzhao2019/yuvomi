          ...
          <div class="allday-cell">
            ...
            ${alldayEvs[i].map((ev) => {
              ...
              <div class="allday-event${eventCompletionClass(ev)}" data-id="${ev.id}" data-calendar-event
                   style="${eventSurfaceStyle(ev)}"
                   title="${allDayChipTitle(ev, timeText)}">${renderEventCompletionControl(ev)}${eventIconHtml(ev.icon, 'event-icon event-icon--compact')}${calendarRepeatIconHtml(ev)}<span class="allday-event__label"><span>${esc(ev.title)}</span>${allDayChipTimeHtml(timeText)}</span>${chipAssigneeStack(ev, { size: 16, maxVisible: 3 })}</div>
            `;
            }).join('')}
