...
import { parseRemindAtAsUtc, naiveUtc, wallTimeToInstant } from '/utils/reminder-offset.js';
...
function matchesPeopleFilter(item) {
  ...
  return zugewiesen.some((u) => state.people.has(Number(u.id)));
}
...
function renderTaskChip(task, { interactive = true, agenda = false, agendaDate = '', icon = true } = {}) {
  ...
  const check = icon ? `<button type="button"
...
function renderWeekEvent(ev, layout = null, dayStr = null) {
  ...
  return `
    <div class="week-event${eventCompletionClass(ev)}" data-calendar-event data-id="${ev.id}"
      ...
      <div class="week-event__title">${renderEventCompletionControl(ev)}${eventGlyphsHtml(ev)}<span class="cal-event__title">${esc(ev.title)}</span>${chipAssigneeStack(ev, { size: 14, maxVisible: 2 })}</div>
  ...
  container.insertAdjacentHTML('beforeend', `
    <div class="agenda-view page-scrollport" id="agenda-view">
      ${groups.length === 0
        ...
        : groups.map(({ date, events, tasks, entries, holidays, schedule, waste }) => `
...
async function openEventDetail(ev, anchor = null) {
  ...
  if (ev.external_source === 'ics'
      && (ev.user_modified === 1 || ev.reminder_suppressed === 1)
      && !readOnly()) {
    actions.push({
      id: 'detail-ics-reset',
...
const REMINDER_PRESET_MINUTES = [0, 15, 30, 60, 1440, 2880, 10080, 20160];
...
function reminderStartMs(startDatetime, event = null) {
  return reminderStartInstant(startDatetime, event).getTime();
}
...
function reminderOffsetFromEvent(event, reminder) {
  ...
  return reminderRowForLead(reminderLeadMinutes(reminderStartMs(anchor, event), reminder.remind_at)).offset;
}
...
function customReminderFromEvent(event, reminder) {
  ...
  const { amount, unit } = reminderRowForLead(reminderLeadMinutes(reminderStartMs(anchor, event), reminder.remind_at));
  return { amount, unit };
}
...
function reminderTimesFromOffsets(offsets, anchorStart, event = null) {
  const startMs = reminderStartInstant(anchorStart, event).getTime();
 ...
 */
function reminderTimesFromRows(rows, anchorStart, enabled = true, event = null) {
  if (!enabled) return [];
  const times = [];
  for (const row of rows) {
    let at = null;
    if (keepsStoredInstant(row)) {
      ...
      [at] = reminderTimesFromOffsets([offset], anchorStart, event);
    }
 ...
 */
function afterStartResolution(storedAt, { anchorStart, openedAt, currentStart, event = null } = {}) {
  if (!storedAt || !anchorStart || !openedAt || !currentStart) return null;
  const anchorMs = reminderStartMs(anchorStart, event)
    + (reminderStartMs(currentStart, event) - reminderStartMs(openedAt, event));
 ...
 */
function syncReminderAfterStart(panel, { anchorStart = null, openedAt = '', event = null } = {}) {
  ...
  for (const row of rows) {
    ...
    const next = afterStartResolution(storedAt, { anchorStart, openedAt, currentStart, event });
    if (next) {
...
function wireReminderRows(panel, { event = null } = {}) {
  ...
  const afterStartContext = {
    anchorStart: reminderAnchorStart(event) ?? null,
    openedAt: eventStartFromForm(panel),
    event,
  };
...
function buildEventModalContent({ mode, event, date, reminder = null, time = null }) {
  ...
  const advancedFieldsOpen = isEdit && (
    (state.users.length > 1 || othersCanRead('calendar')) && visibility !== 'all'
    || !!event.countdown
    || hasAttachment(event)
...
async function saveEvent(overlay, mode, event, existingReminder = null, attachmentState = null) {
  ...
  try {
    ...
    if (savedEventId && !remindersHandledAtomically) {
      ...
      const remindAts = reminderTimesFromRows(reminderForm.rows, reminderBaseStart, reminderForm.enabled, event);
      if (remindAts.length) {
...
async function deleteEvent(event) {
  ...
  scheduleCalendarDeleteWithUndo({
    ...
    requestDelete: async ({ keepalive }) => {
      // Die Erinnerungen des Termins raeumt der Server mit ab (Migration v227,
