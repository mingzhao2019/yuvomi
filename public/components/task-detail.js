...
export async function deleteTaskWithUndo(id, { container = null, onChanged = () => {} } = {}) {
  ...
  scheduleUndoableDelete({
    ...
    commit: async ({ keepalive }) => {
      // Die Erinnerungen der Aufgabe raeumt der Server mit ab (Migration v227,
...
function taskListLabel(list, taskLists = []) {
  if (!list) return t('tasks.taskListLocal');
  const name = list.name || t('tasks.taskListLocal');
  const providerAccounts = new Set(
    taskLists
      .filter((item) => item?.provider === list.provider)
      .map(taskListAccountKey)
      .filter(Boolean),
  );
  // The tasks page hides a sole account name to keep cards compact. A detail
  // opened from another module may not have the list collection in memory, so
  // retain the account there when the API supplied it to avoid ambiguity.
  const showAccount = !!list.account_name
    && (providerAccounts.size > 1 || (!taskLists.length && list.provider !== 'local'));
  return showAccount ? `${list.account_name} · ${name}` : name;
}
