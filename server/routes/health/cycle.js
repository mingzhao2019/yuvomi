...
// Geschlossene Werte-Listen fuer die seit Migration 220 nullbaren Spalten
...
/**
 * Gefuehle eines Tages (Mehrfachauswahl, seit Migration 221) validieren +
...
router.get('/cycle/logs', (req, res) => {
  try {
    ...
    // `symptoms`/`feelings` kommen aus je einer eigenen Tabelle (Migration 182
    // bzw. 221), nicht mehr aus den (nur noch historischen) Skalar-Spalten -
    ...
    // als reiner Altlast-Lesewert stehen (siehe Migration 221). Batch statt
    // einer Abfrage je Zeile (symptomsForLogs()/feelingsForLogs(), je ein
    // `IN (...)`). Die Symptome stammen im custom-Schema aus Migration 182;
    // die Gefuehle aus Migration 221.
    ...
    res.json({ data: rows.map((row) => {
      ...
      // ist - siehe Migration 220's Kommentar und docs/SPEC.md. Sex-Leben
...
// Kein CHECK auf der Spalte (siehe Migration 222) - dieselbe Aufteilung wie
