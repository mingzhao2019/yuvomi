...
import { householdTimeZone, shiftDateKey, utcToWall } from '../../utils/timezone.js';
import {
  StorageError,
  cleanupStagedUpload,
  readDocumentContent,
...
router.get('/:id', (req, res) => {
  try {
    ...
    const [decorated] = decorateEventCompletions(database, [resolved], getUserId(req));
    res.json({ data: serializeEvent(decorated, {
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const database = db.get();
    const resolved = resolveProjectedEventRows(database, [event])[0];
    const [decorated] = decorateEventCompletions(database, [resolved], userId);
    res.status(201).json({ data: serializeEvent(decorated, {
      viewer: documentViewer(req),
      database,
      actorId: getUserId(req),
      isAdmin: isAdminUser(req),
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    const database = db.get();
    const resolved = resolveProjectedEventRows(database, [updated])[0];
    const [decorated] = decorateEventCompletions(database, [resolved], getUserId(req));
    res.json({ data: serializeEvent(decorated, {
      viewer: documentViewer(req),
      database,
...
router.delete('/:id', (req, res) => {
  try {
    ...
    // Die Erinnerungen abzuraeumen ist seit Migration v227 Sache eines
