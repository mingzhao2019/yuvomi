...
export function serializeEvent(event, context = null) {
  ...
  const normalizedContext = context && typeof context.prepare === 'function'
    ? { database: context }
    : context;
  const database = normalizedContext?.database ?? null;
  ...
  const visibleDocumentIds = normalizedContext?.visibleAttachmentDocumentIds
    ?? visibleAttachmentDocumentIds([event], normalizedContext);
  ...
  const attachmentLocked = normalizedContext?.viewer?.readsDocuments === true ? documentHidden : null;
  const metadata = recurrenceMetadata(event, normalizedContext);
  return {
    ...rest,
