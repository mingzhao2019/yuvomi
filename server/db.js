...
const MIGRATIONS = [
  ...
  },
  {
    ...
    up: `
      ...
      -- Remote providers use external_account_id plus either external_list_id or
      -- external_list_url; local lists do not need an external identity.
      ...
      -- CalDAV collections are stable by account and URL. The partial index
      -- leaves room for multiple local lists with no external identity.
      ...
      -- Preserve the list identity for installations that already have a
      -- CalDAV task selection. The next sync fills in tasks whose object URL was
      -- not available when an older version imported them.
  ...
  {
    version: 206,
    description: 'Shopping: change counter per list, fed by triggers, for live updates',
    up: `
  ...
  },
  {
    ...
    // kind's CHECK only allowed 'file' (migration 209); widening it to add
  ...
  },
  {
    ...
    // through its own ON DELETE CASCADE (same hazard as migrations 52/212/213).
    //
  ...
  },
  {
    ...
    // idx_waste_imported_pickups_type_date (migration 211) is keyed on
  ...
  },
  {
    ...
    // spalten-eigenen CHECK tragen (siehe Migration 222, perimenopause_mode/
  ...
  },
  {
    ...
    // Gleiches Muster wie Migration 182 (cycle_day_log_symptoms): die alte
    ...
    up: `
      ...
      -- Migration 182 (Komma-Liste, mehrere Symptome je Zeile) ist hier keine
  ...
  },
  {
    ...
    up: `
      ...
      -- v228 (Dokumente) hat 'document_expiry' bereits eigenstaendig
      ...
      -- bereits in v228 dokumentiert: ihr Koerper nennt "reminders" beim
      ...
      -- wie in v228 (Review #1255 nice-to-have): kostet hier nichts, haelt
  ...
  },
  {
    ...
    foreignKeysOff: true,
    up: `
      -- Carries v229's 'health_prevention_due' forward: this rebuild replaces
      -- the table v229 just created, so every value it allowed has to be listed
  ...
  },
  {
    ...
    // v234 hat den Autor der noch vorhandenen Ledger-Zeilen korrigiert; die
    ...
    up(db) {
      db.exec(`
        DROP TABLE IF EXISTS temp._v235_missing;
        CREATE TEMP TABLE _v235_missing AS
      ...
      const rebuilt = db.prepare(`
        SELECT e.id, e.group_id FROM expenses e JOIN _v235_missing m ON m.id = e.id ORDER BY e.id
      `).all();
      db.exec(`
        ...
        FROM (
          ...
          FROM expenses e JOIN _v235_missing m ON m.id = e.id
          UNION ALL
          SELECT e.id, s.id, e.group_id, s.user_id, e.payer_id, -s.amount_minor, s.currency, e.title, e.created_by
          FROM expenses e JOIN _v235_missing m ON m.id = e.id JOIN expense_splits s ON s.expense_id = e.id
        ) x
        ORDER BY x.id, x.ord;
