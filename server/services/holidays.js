 ...
 * Neben OpenHolidays bleiben China und die lokal berechneten Länder auch bei
 * einem API-Ausfall auswählbar.
 * @returns {Promise<{countries: Array<{isoCode: string, name: string, schoolHolidays?: boolean}>, partial: boolean}>}
 */
async function getCountryCatalog() {
  ...
  try {
    ...
    // 中国法定节假日和本地规则国家不应因为 OpenHolidays 暂时不可用而
    // 让设置页失去可用选项。其它国家仍会在实际同步时按原策略重试。
  ...
  const byCode = new Map(apiCountries.filter((c) => c.isoCode).map((c) => [c.isoCode, c]));
  for (const country of BUILTIN_COUNTRIES) {
    if (!byCode.has(country.isoCode)) byCode.set(country.isoCode, country);
  }
  for (const isoCode of Object.keys(LOCAL_COUNTRIES)) {
    if (!byCode.has(isoCode)) {
      byCode.set(isoCode, {
        isoCode,
        name: LOCAL_COUNTRIES[isoCode].name,
...
async function syncYearAndType(country, subdivision, year, type, langCode) {
  ...
  } else if (type === 'public' && LOCAL_COUNTRIES[country]) {
