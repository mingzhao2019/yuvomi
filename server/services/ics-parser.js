...
function parseICS(ics, { onSkip, allowMissingUid = false } = {}) {
  ...
  while ((match = vEventRe.exec(unfolded)) !== null) {
    ...
    // Keep both the calendar reminder projection and Waste's import metadata.
    ...
    events.push({ uid, summary, description, location, dtstart, dtend, rrule, allDay, color, hasColor, exdates, recurrenceId, tzid, alarms, categories, status, hasRDate });
  }
  return events;
}
