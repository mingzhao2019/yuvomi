...
export async function processDueNotifications({
  ...
  const due = activeDb.prepare(`
    ...
    WHERE r.dismissed = 0 AND r.pushed_at IS NULL AND r.remind_at <= ?
      ...
      -- Migration v227 raeumen zwei AFTER-DELETE-Trigger diese Erinnerungen mit
  ...
  // Einmal je Lauf, nicht je Meldung: Sprache, Datumsformat und Zeitzone
  // gehoeren dem Haushalt.
  const { locale, dateFormat } = resolveHouseholdFormats(activeDb);
  const timeZone = householdTimeZone(activeDb);

  // Nur fuer eine tatsaechliche Partner-Erinnerung geholt - die Sammelabfrage
  // soll weiterhin mit den minimalen Fixture-Tabellen der Tests funktionieren.
  ...
  for (const reminder of due) {
    ...
    const payload = reminderPayload(reminder, locale, nowIso, timeZone, dateFormat);
